...
fu.staticHandler = function (filename) {
	...
	//sys.puts("content type: " + content_type);
...
//////////////////////////////////////////////////////////////////////////
// Runs staticHandler for a whole directory
fu.staticDirHandler = function (dirname) {
	sys.puts("reading directory: " + dirname);
	readdir(dirname, function(err,files) {
		if (err) {
			sys.puts("Error loading dir: " + dirname);
		} else {
			for (x = 0; x < files.length; ++x) {
				//sys.puts("A file in dir is: " + files[x]);
				fu.get("/" + dirname + files[x], fu.staticHandler(dirname + files[x]));
			}
		}
	});
};
