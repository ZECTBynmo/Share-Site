...
function initUI() {
	...
	// Vote buttons
	// ---------------------------------------------------------------------------------------------
	$("#buttonVoteDown").button({});
	$("#buttonVoteUp").button({});
	
	// ---------------------------------------------------------------------------------------------
	// Accept button
	// ---------------------------------------------------------------------------------------------
	$("#buttonAcceptNext").button({});
	
	var onClickAccept = function(e) {
		// Get reference to all entries in the user's queue
		
		var userVidQueue = [];
		$("ul li").each(function() { 
			var vidID = $(this).attr("vidID");
		
			// Only do stuff if we've got a queue item
			if( vidID ) {
				userVidQueue.push( vidID );
			}
		});
		
		if( userVidQueue.length > 0 ) {
			// Alert the server that we're playing the first one
			var strVidID = userVidQueue[0];
			alertNextVidInfo( strVidID, 60*10 );
		}
	}
	
	$("#buttonAcceptNext").on('click', onClickAccept);
	
	// ---------------------------------------------------------------------------------------------
	// Decline button
	// ---------------------------------------------------------------------------------------------
	$("#buttonDeclineNext").button({});
	
	var onClickDecline = function(e) {
		alertNextUser();
	};
	
	$("#buttonDeclineNext").on('click', onClickDecline);
	
	// ---------------------------------------------------------------------------------------------
	// Add to queue button
	// ---------------------------------------------------------------------------------------------
	$("#buttonAddToQueue").button({});
	
	var onClickAddToQueue = function(e) {
		alertAddSelfToQueue();
	};
	
	$("#buttonAddToQueue").on('click', onClickAddToQueue);
	
	// Create a YUI sprogress bar
	YUI().use('yui2-progressbar',function(Y){
		var YAHOO = Y.YUI2;
		var pb = new YAHOO.widget.ProgressBar({
			value:50,
			height:15
		}).render('progBar'); 
		// We want to animate between values to look slick
		pb.set('anim', true);
		
		// Set our width to the width of the video - 2 (because of the outline on the progress bar
		pb.set( 'width', screen.width / 3 - 2 );
		
		// Initialize the progress value to zero
		pb.set( 'value', 0 );
		
		// Start a timer that checks the time frequently
		var progressTimer= setInterval(function() {
			
			if( CONFIG.canReceiveMessages ) {
				try {
					var totalDuration= m_vidPlayer.getDuration();
					var currentTime= m_vidPlayer.getCurrentTime();
					var currentPercentage= 100 * currentTime / totalDuration;
					
					pb.set( 'value', currentPercentage );		
				} catch(err) {
	...
	YUI().use('yui2-slider',function(Y){
		var YAHOO = Y.YUI2;
		slider = YAHOO.widget.Slider.getVertSlider( "volumeSlider", "sliderthumb", 0, 210 );
		
		var sliderDiv = document.getElementById( "volumeSlider" );
		
		// Default to 50 percent for the slider
		var currentVolume = 50;
		
		try {
			currentVolume = 100 - ( m_vidPlayer.getVolume() - sliderDiv.clientHeight );
		} catch(err) {
		}
		...
		slider.on( 'change', function(e) {				
			try {
				...
				// Get the value of the slider (reversed and corrected for its height)
				var sliderValue = slider.getValue();
				var sliderPercent = 100 * ( sliderDiv.clientHeight - sliderValue - 10 ) / sliderDiv.clientHeight;
				
				if( sliderPercent < 100 )
					m_vidPlayer.setVolume( sliderPercent );	
			} catch( err ) {
		...
		for (var i = 0; i < data.messages.length; i++) {
			...
			switch (message.type) {
				...
				case "vidStart":
					...
					if( message.data )
						vidStartTime = message.data/1000;		// We get the time in milliseconds
					
					var numPlayTries= 0;
					
					if( message.text.length > 0 ) {
						m_vidPlayer.forcePlayVideoById( message.text, vidStartTime, 2000 );
					}
				break;
				case "vidQueue":
					if( message.text.length > 0 ) {
						m_vidPlayer.forcePlayVideoById( message.text, vidStartTime );
					}
				/*
					var vidPlayer= $("#myytplayer").get(0);
					var vidStartTime = 0;
					
					if( message.data )
						vidStartTime = message.data/1000;		// We get the time in milliseconds
					
					if( vidPlayer && message.text.length > 0 ) {
						try {
							// Try to queue the video right away
							vidPlayer.loadVideoById( message.text, vidStartTime );
						}
						
						catch(err) {
							// We failed to queue the video, start a timer and 
							// try again every 2 seconds until we succeed
							var vidQueueTimer= window.setInterval(function() {
								var didSucceed= true;
								
								try {
									// Don't bother trying to queue the video if we're starting after it has ended
									if( vidStartTime > vidPlayer.getDuration() ) {
										window.clearInterval(vidQueueTimer);
									} else {									
										// Load the video
										vidPlayer.loadVideoById( message.text, vidStartTime );
									}
				...
				*/
				...
				break;
				case "nextUser":
					// Update our current user label
					var currentPlayerLabel = $("#currentUserLabel").get(0);
					currentPlayerLabel.textContent = message.text;
					var strCurrentPlayer= currentPlayerLabel.textContent;
					
					// If we're the next user, show the controls
					if( message.text == CONFIG.nick ) {
						currentPlayerLabel.textContent = "It's your turn, press yes to play a video";
						$("#buttonAcceptNext").show();
						$("#buttonDeclineNext").show();
					} else {
						currentPlayerLabel.textContent = message.text + " is in control";
						$("#buttonAcceptNext").hide();
						$("#buttonDeclineNext").hide();
					}
				break;
				case "userQueueChanged":
					getUserQueue();
				break;
			}
...
function onConnect (session) {
	...
	if( CONFIG.isYoutubePlayerInitialized ) {
		longPoll();
	} else {
		var longPollInterval = setInterval(function () {
			if( CONFIG.isYoutubePlayerInitialized ) {
				window.clearInterval( longPollInterval );
				longPoll();
			}
		}, 1000 );
	}
